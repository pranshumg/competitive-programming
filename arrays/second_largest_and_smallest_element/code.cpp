...
int second_largest_element_opt(vector<int>& v, int n) {
  int largest = v[0], second_largest = INT_MIN;
  for (int i = 1; i < n; i++) {
    if (v[i] > largest) {
...
// Better (TC - O(n), SC - O(1))
int second_smallest_element_better(vector<int>& v, int n) {
...
// Optimal (TC - O(n), SC - O(1))
int second_smallest_element_opt(vector<int>& v, int n) {
  int smallest = v[0], second_smallest = INT_MAX;
  for (int i = 1; i < n; i++) {
    if (v[i] < smallest) {
